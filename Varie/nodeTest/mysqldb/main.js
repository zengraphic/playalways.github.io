...
app.get('/WDlinks/:id', function(req, res) {
    connection.query('select * from WDlinks where id=?', [req.params.id], function(error, results, fiel$
        if (error) throw error; res.end(JSON.stringify(results));
    });
});
...
app.put('/WDlinks', function(req, res) {
    connection.query('UPDATE `WDlinks` SET `link_title`=?,`link_url`=?,`link_color`=? where `id`=?', [re$
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
...
app.delete('/WDlinks', function(req, res) {
    ...
    connection.query('DELETE FROM `WDlinks` WHERE `id`=?', [req.body.id], function(error, results, fiel$
        if (error) throw error; res.end('Record has been deleted!');
    });
});
