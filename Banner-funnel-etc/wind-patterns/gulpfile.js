...
gulp.task('pl-sass-wind', function() {
    return gulp.src(path.resolve(paths().source.css, 'wind-style.scss'))
...
gulp.task('pl-copy:maps', function() {
    return gulp.src(resolvePath(paths().source.css) + '/*.map')
        .pipe(gulp.dest(resolvePath(paths().public.css)))
        .pipe(browserSync.stream());
});
...
gulp.task('pl-assets', gulp.series(
    gulp.parallel(
        ...
        'pl-copy:favicon',
        'pl-copy:font',
        gulp.series(gulp.parallel('pl-sass-wind'), 'pl-copy:css','pl-copy:maps', function(done) { done(); }),
        'pl-copy:styleguide',
...
function watch() {
    ...
    gulp.watch(resolvePath(paths().source.css) + '/**/*.css', { awaitWriteFinish: true }).on('change', gulp.series('pl-copy:css','pl-copy:maps', reloadCSS));
